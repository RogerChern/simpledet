...
def get_config(is_train):
    ...
    class NormalizeParam:
        normalizer = normalizer_factory(type="localbn", wd_mult=1.0, mom=0.99)
    ...
    class OptimizeParam:
        ...
        class schedule:
            begin_epoch = 55
            end_epoch = 80
            lr_iter = [(14786 * end_epoch - 60000) * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image),
                       (14786 * end_epoch - 20000) * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)]

        class warmup:
            type = "gradual"
            lr = 0.001 / 8 * len(KvstoreParam.gpus) * KvstoreParam.batch_image
            iter = int(14786 * 80 * 0.05) * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)
            in_pct = True
    ...
    # override pretrain model if resuming
    if OptimizeParam.schedule.begin_epoch != 0:
        ModelParam.pretrain.prefix = TestParam.model.prefix
        ModelParam.pretrain.epoch = OptimizeParam.schedule.begin_epoch
