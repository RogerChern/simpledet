from models.KD.builder import FitNetFasterRcnn as Detector
...
def get_config(is_train):
    ...
    detector = Detector()
    if is_train:
        train_sym = detector.get_train_symbol(backbone, neck, rpn_head, roi_extractor, bbox_head, kd_head)
        rpn_test_sym = None
        test_sym = None
    else:
        train_sym = None
    ...
    scalar_metric = metric.ScalarLoss(
        "fitloss",
        ["fit_loss_output"],
        []
    )

    metric_list = [rpn_acc_metric, rpn_l1_metric, box_acc_metric, box_l1_metric, scalar_metric]
