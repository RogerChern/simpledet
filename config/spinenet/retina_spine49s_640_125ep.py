...
from models.spinenet.builder import SpineNet49s as Backbone
...
def get_config(is_train):
    class General:
        ...
        batch_image = 12 if is_train else 1
        fp16 = True
    ...
    class NormalizeParam:
        normalizer = normalizer_factory(type="localbn", mom=0.99)
    ...
    class OptimizeParam:
        ...
        class schedule:
            begin_epoch = 0
            end_epoch = 125
            lr_iter = [14652 * 105 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image),
                       14652 * 115 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image),
                       14652 * 120 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)]

        class warmup:
            type = "gradual"
            lr = 0.0001 / 8 * len(KvstoreParam.gpus) * KvstoreParam.batch_image
            iter = 14652 * 5 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)
