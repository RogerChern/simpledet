...
from models.tridentnet.builder_pami import OFANeck
...
def get_config(is_train):
    ...
    class OFAParam:
        fp16 = General.fp16
        no_grad_for_teacher = True
        use_relu_in_transform = True
        target_channels = [1024, 1024, 1024]
        grad_scales = [10, 10, 10]
        student_endpoints = ["stage3_unit6_relu_branch0_output", "stage3_unit6_relu_branch1_output", "stage3_unit6_relu_branch2_output"]
        teacher_endpoints = ["stage3_unit36_relu_branch0_output", "stage3_unit36_relu_branch1_output", "stage3_unit36_relu_branch2_output"]

    class NeckParam:
        fp16 = General.fp16
        normalizer = NormalizeParam.normalizer
        student_endpoints = OFAParam.student_endpoints

    class RpnParam:
        fp16 = General.fp16
        normalizer = NormalizeParam.normalizer
        batch_image = General.batch_image * Trident.num_branch * (2 if is_train else 1)
    ...
    if is_train:
        train_sym = detector.get_train_symbol(
            backbone, ofa_neck, rpn_head, roi_extractor, bbox_head, ofa_head,
