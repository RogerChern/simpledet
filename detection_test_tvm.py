...
if __name__ == "__main__":
    ...
    # sym = mx.sym.load(pTest.model.prefix + "_test.json")
    ...
    v_roidb, h_roidb = AnchorLoader.roidb_aspect_group(roidbs_all)
    ...
    for index_split in range(int(math.ceil(len(roidbs_all) / split_size))):
        ...
        if index_split == 0:
            def eval_worker(i, exe, data_queue, result_queue):
                while True:
                    batch = data_queue.get()
                    inputs = {k: tvm.nd.array(d.asnumpy(), ctx=tvm.gpu(i)) for d, (k, shape) in zip(batch.data, batch.provide_data)}
                    exe.run(**inputs)
                    out = [exe.get_output(i).asnumpy() for i in range(exe.get_num_outputs())]
                    result_queue.put(out)
            for i, exe in zip(pKv.gpus, execs):
                workers.append(Thread(target=eval_worker, args=(i, exe, data_queue, result_queue)))
            for w in workers:
                w.daemon = True
                w.start()
