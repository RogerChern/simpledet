 ...
 */
#ifndef MXNET_OPERATOR_PROPOSAL_TARGET_INL_H_
#define MXNET_OPERATOR_PROPOSAL_TARGET_INL_H_
...
namespace proposal_target_enum {
enum ProposalTargetInputs {kRois, kGtBboxes};
...
class ProposalTargetOp : public Operator {
 public:
  explicit ProposalTargetOp(ProposalTargetParam param) {
    this->param_ = param;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    CHECK_GT(req.size(), 4);
    CHECK_EQ(req[proposal_target_enum::kRoiOutput], kWriteTo);
    ...
    Tensor<xpu, 2, DType> xpu_labels       = out_data[proposal_target_enum::kLabel].
                                             get_with_shape<xpu, 2, DType>(Shape2(num_image, image_rois), s);
    Tensor<xpu, 3, DType> xpu_bbox_targets = out_data[proposal_target_enum::kBboxTarget].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, param_.num_classes * 4), s);
    Tensor<xpu, 3, DType> xpu_bbox_weights = out_data[proposal_target_enum::kBboxWeight].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, param_.num_classes * 4), s);
    Tensor<xpu, 2, DType> xpu_match_gt_idxes = out_data[proposal_target_enum::kMatchGtIdx].
                                             get_with_shape<xpu, 2, DType>(Shape2(num_image, image_rois), s);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Stream<xpu> *s = ctx.get_stream<xpu>();
    Tensor<xpu, 3, DType> rois      = in_grad[proposal_target_enum::kRois].get<xpu, 3, DType>(s);
    Tensor<xpu, 3, DType> gt_bboxes = in_grad[proposal_target_enum::kGtBboxes].get_with_shape<xpu, 3, DType>(Shape3(num_image, num_gtbbox, 5), s);
...
Operator *CreateOp(ProposalTargetParam param, int dtype);
