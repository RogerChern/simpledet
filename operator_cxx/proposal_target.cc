...
template<>
Operator *CreateOp<cpu>(ProposalTargetParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalTargetOp<cpu, DType>(param);
  })
  return op;
}

template<>
Operator *CreateOp<gpu>(ProposalTargetParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalTargetOp<gpu, DType>(param);
  })
  return op;
}

Operator *ProposalTargetProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
MXNET_REGISTER_OP_PROPERTY(ProposalTarget, ProposalTargetProp)
