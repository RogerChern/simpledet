import numpy as np
from .cython.cpu_nms import soft_nms
...
def nms_bbox_vote_wrapper(nms_thresh, vote_thresh, score_thresh=0):
    def _nms(dets):
        nmsed_dets = nms(dets, nms_thresh)
        dets[dets[:, 4] < score_thresh, 4] *= 1e-4
        return box_voting(nmsed_dets, dets, vote_thresh)
    return _nms


def soft_nms_bbox_vote_wrapper(nms_thresh, vote_thresh, score_thresh=0):
    def _nms(dets):
